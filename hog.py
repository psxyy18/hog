...
def is_swap(player_score, opponent_score):
    ...
    abs_diff = abs(player_one_digit - opponent_one_digit)
    if abs_diff == opponent_ten_digit:
        return True
    else:
        return False
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    """
    # BEGIN PROBLEM 9
    make_averaged_roll_dice = make_averaged(roll_dice, trials_count)
    max_average = 0
    optimal_num_rolls = 1

    for num_rolls in range(1,11):
        average_score = make_averaged_roll_dice(num_rolls, dice)
...
def swap_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    will_swap = is_swap(potential_score, opponent_score)
    if will_swap and potential_score < opponent_score:
        return 0
    if free_bacon(opponent_score) >= cutoff and not(will_swap and potential_score > opponent_score):
        return 0
    return num_rolls
